...
arg_scope = tfa.framework.arg_scope
...
def nasnet_cifar_arg_scope(weight_decay=5e-4,
  ...
  weights_regularizer = tfa.layers.l2_regularizer(weight_decay)
  weights_initializer = tfa.layers.variance_scaling_initializer(
      mode='FAN_OUT')
...
def _build_aux_head(net, end_points, num_classes, hparams, scope):
  ...
  with tf.compat.v1.variable_scope(scope):
    ...
    with tf.compat.v1.variable_scope('aux_logits'):
      ...
      aux_logits = tf.nn.relu(aux_logits)
      aux_logits = tfa.layers.flatten(aux_logits)
